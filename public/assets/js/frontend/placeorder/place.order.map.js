// 只显示起终点
function map1(myP1, myP2,callBackFun) {
    // 地图
    ...
    var myIcon1 = new BMap.Icon("/assets/img/icon-mai.png", new BMap.Size(25, 64), { //起点图片
        imageOffset: new BMap.Size(0, 0) //图片的偏移量。为了是图片底部中心对准坐标点。
    });

    var myIcon2 = new BMap.Icon("/assets/img/icon-shou.png", new BMap.Size(25, 64), { //终点图片
        imageOffset: new BMap.Size(0, 0) //图片的偏移量。为了是图片底部中心对准坐标点。
    });
...
function getRange1(map, myP1, myP2 , callBackFun) {
    ...
    var searchComplete = function(results) {
        ...
        callBackFun({'d':d, 'h':h, 'm':m, 'distance':distance});
...
// 带跑男获取两点间的距离和时间
function getRange(map, myP1, myP2) {
    var output = ''
    var searchComplete = function(results) {
        if (transit.getStatus() != BMAP_STATUS_SUCCESS) {
            return;
